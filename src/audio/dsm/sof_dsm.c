	...
	void *in, void *out, int nSamples, int szSample)
{
	...
	if (*wrPtr + nSamples <= DSM_FF_BUF_SZ) {
		if (is_16bit)
		...
		*wrPtr += nSamples;
		...
		comp_info(dev, "[DSM] FF process buffer overflow. rdPtr : %d",
			*wrPtr);
		return;
	}
	...
	if (*wrPtr >= DSM_FF_BUF_SZ) {
		...
		for (x = 0; x < DSM_FRM_SZ; x++) {
			...
			int pos = (*wrPtr - DSM_FF_BUF_SZ + x);

			if (is_16bit) {
				input[x] = (buf.buf16[2 * pos]);
				input[x + DSM_FRM_SZ] =
					(buf.buf16[2 * pos + 1]);
			} else {
				input[x] = (buf.buf32[2 * pos] >> 16);
				input[x + DSM_FRM_SZ] =
					(buf.buf32[2 * pos + 1] >> 16);
			}
		}
		...
		for (x = 0; x < DSM_FRM_SZ; x++) {
			...
			if (is_16bit) {
				buf_out.buf16[2 * pos] = (output[x]);
				buf_out.buf16[2 * pos + 1] =
					(output[x + DSM_FRM_SZ]);
			} else {
				buf_out.buf32[2 * pos] = (output[x] << 16);
				buf_out.buf32[2 * pos + 1] =
	...
	void *in, int nSamples, int szSample)
{
	...
	if (*wrPtr + nSamples <= DSM_FB_BUF_SZ) {
		if (is_16bit)
			memcpy_s(&buf.buf16[*wrPtr], nSamples * szSample,
				in, nSamples * szSample);
		else
	...
	if (*wrPtr >= DSM_FB_BUF_SZ) {
		...
		for (x = 0; x < DSM_FRM_SZ; x++) {
			...
			int pos = (*wrPtr - DSM_FB_BUF_SZ + x);

			if (is_16bit) {
				v[x] = buf.buf16[4 * pos + 1];
				i[x] = buf.buf16[4 * pos];
				v[x + DSM_FRM_SZ] = buf.buf16[4 * pos + 3];
				i[x + DSM_FRM_SZ] = buf.buf16[4 * pos + 2];
			} else {
				v[x] = (buf.buf32[4 * pos + 1] >> 16);
				i[x] = (buf.buf32[4 * pos] >> 16);
				v[x + DSM_FRM_SZ] =
					(buf.buf32[4 * pos + 3] >> 16);
				i[x + DSM_FRM_SZ] =
					(buf.buf32[4 * pos + 2] >> 16);
			}
		}
