...
int sof_dsm_inf_reset(struct sof_dsm_struct_t *hsof_dsm, struct comp_dev *dev)
{
	...
	memset(hsof_dsm->buf.sof_a_frame_in, 0,
	       SOF_FF_BUF_DB_SZ * sizeof(int32_t));
	memset(hsof_dsm->buf.sof_a_frame_out, 0,
	       SOF_FF_BUF_DB_SZ * sizeof(int32_t));
	memset(hsof_dsm->buf.sof_a_frame_iv, 0,
				  ...
				  int num_channel)
{
	...
	case SOF_IPC_FRAME_S16_LE:
		for (x = 0 ; x < frames ; x++)            {
			for (y = 0 ; y < num_channel ; y++)            {
				input.buf16 =
					(int16_t *)_wrap_buf_ptr(input.buf16,
								 stream);
	...
	case SOF_IPC_FRAME_S32_LE:
		for (x = 0 ; x < frames ; x++)            {
			for (y = 0 ; y < num_channel ; y++)            {
				input.buf32 =
					(int32_t *)_wrap_buf_ptr(input.buf32,
								 stream);
		...
		for (x = 0 ; x < frames ; x++)           {
			for (y = 0 ; y < num_slot ; y++)            {
				if (y == 0)
					offset = 0;
				output.buf16 =
					(int16_t *)_wrap_buf_ptr(output.buf16,
								 stream);
				value = pow_2[y] & mask;
				*output.buf16 =
		...
		for (x = 0 ; x < frames ; x++)            {
			for (y = 0 ; y < num_slot ; y++)            {
				if (y == 0)
					offset = 0;
				output.buf32 =
					(int32_t *)_wrap_buf_ptr(output.buf32,
								 stream);
				value = pow_2[y] & mask;
				*output.buf32 =
