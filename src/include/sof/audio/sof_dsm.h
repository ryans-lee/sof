...
struct sof_dsm_prep_ff_buf_struct_t {
	int32_t buf[DSM_FF_BUF_SZ];
	int avail;
	int rdy;
};
struct sof_dsm_prep_fb_buf_struct_t {
	int32_t buf[DSM_FB_BUF_SZ];
	int avail;
	int rdy;
};
struct sof_dsm_buf_struct_t {
	/* sof -> dsm FF */
	int32_t sof_a_frame_in[SOF_FF_BUF_SZ];
	/* sof <- dsm FF */
	int32_t sof_a_frame_out[SOF_FF_BUF_SZ];
	/* sof -> dsm FB */
	int32_t sof_a_frame_iv[SOF_FB_BUF_SZ];
	/* stage buffer */
	int32_t stage[DSM_FF_BUF_SZ];
	/* dsm FF in */
	int16_t *input;
	/* dsm FF out */
	int16_t *output;
